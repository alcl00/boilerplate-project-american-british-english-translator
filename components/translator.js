...
class Translator {
    ...
    translate(text, dict, titles, timeRegex, locale) {
        ...
        lowerCase.match(/(\w+([-'])(\w+)?['-]?(\w+))|\w+/g)
        .map((word) => {
            if(dict[word]) return matchesMap[word] = dict[word];
        })

        const matchedTimes = lowerCase.match(timeRegex);
    ...
    replaceAll(text, matchesMap) {
        const replacement = new RegExp(Object.keys(matchesMap).join("|"), "gi")
        return text.replace(replacement, (matched) => 
